/**
 * FadeFrame.jsx — Vite Migration Refactor (2026)
 *
 * Purpose:
 * Renders the story fade-frame interaction with:
 * - selectable topic buttons
 * - coin comparison panels
 * - optional heading placement
 * - background image
 * - back-to-top navigation
 *
 * Migration Changes Only:
 * 1. File must be renamed from `.js` to `.jsx`
 *    - Required because this file contains JSX and Vite does not allow JSX in `.js`
 *
 * 2. Replaced CRA env usage
 *    - `process.env.REACT_APP_strapiURL` → `import.meta.env.VITE_STRAPI_URL`
 *
 * 3. Removed explicit `.js` extension from Markup import
 *    - Vite-safe import resolution
 *
 * Behavior:
 * - All existing DOM manipulation, fade switching, layout branching,
 *   and rendering behavior are intentionally preserved as-is.
 */
    ...
    buttonOptin.push(
      ...
      <button className='fade-frame-button'
        onClick={(e)=>{
          ...
          let btnsdom = e.target.parentElement.parentElement
          handleBtns(btnsdom,i,props.options.length)
